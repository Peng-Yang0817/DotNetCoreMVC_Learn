using System;
using System.Collections.Generic;

namespace NoteDotNetCoreUseReverseEngineering.Models.TQLDB.Tables;

public partial class ViewOneForm00131
{
    public string Id { get; set; } = null!;

    public int SchoolCtrl001Id { get; set; }

    public string SchoolName { get; set; } = null!;

    public string Tclnum { get; set; } = null!;

    public string SchoolNum { get; set; } = null!;

    public string Year { get; set; } = null!;

    public string ProjectName { get; set; } = null!;

    public int State011Id { get; set; }

    public string? State011Name { get; set; }

    public int SchoolCtrl002Id { get; set; }

    public string? CollegeName { get; set; }

    public string DegreeName { get; set; } = null!;

    public int State012Id { get; set; }

    public string? State012Name { get; set; }

    public int State013Id { get; set; }

    public string? State013Name { get; set; }

    public int State014Id { get; set; }

    public string? State014Name { get; set; }

    public string? State014Year { get; set; }

    public int State015Id { get; set; }

    public string? State015Name { get; set; }

    public string Applicant1 { get; set; } = null!;

    public string Job1 { get; set; } = null!;

    public string Phone1 { get; set; } = null!;

    public string Email1 { get; set; } = null!;

    public string? Applicant2 { get; set; }

    public string? Job2 { get; set; }

    public string? Phone2 { get; set; }

    public string? Email2 { get; set; }

    public string? Applicant3 { get; set; }

    public string? Job3 { get; set; }

    public string? Phone3 { get; set; }

    public string? Email3 { get; set; }

    public double C1 { get; set; }

    public double C2 { get; set; }

    public double C3 { get; set; }

    public double C4 { get; set; }

    public int F3id { get; set; }

    public int SupportNum { get; set; }

    public int? F3c1 { get; set; }

    public int? F3c2 { get; set; }

    public int? F3c3 { get; set; }

    public int? F3c4 { get; set; }

    public int F3sid { get; set; }

    public int? SchoolCtrl004Id { get; set; }

    public string? ShuoBoShrBanName { get; set; }

    public string? ClassName { get; set; }

    public string? GroupName { get; set; }

    public string? CategoryName { get; set; }

    public int? State016Id { get; set; }

    public string? State016Name { get; set; }

    public string? F3sc1 { get; set; }

    public string? F3sc2 { get; set; }

    public string? F3sc3 { get; set; }

    public int? State017Id { get; set; }

    public string? State017Name { get; set; }

    public string? State017Year { get; set; }

    public int? F3sc4 { get; set; }

    public int? F3sc5 { get; set; }

    public int? F3sc6 { get; set; }

    public int? F3sc7 { get; set; }

    public DateTime EditTime { get; set; }

    public int EditAuth001 { get; set; }

    public string? Username { get; set; }

    public string? Name { get; set; }

    public string? OneFormUploadId { get; set; }
}
